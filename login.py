...
def main_login():
    ...
    while log_status:
        ...
        main_menu_open_encrypter = tkinter.Label(open_encrypter_main_menu_gui, text=f"Welcome {user_name}!!", 
            relief="flat", font=("Arial", 25), padx=10)
        writedata_b = tkinter.Button(open_encrypter_main_menu_gui, bg="#20bebe", fg="white", width=30, padx=2, pady=2, font=("Arial", 18), text="Write Data", command=lambda: writedata.datawriter(user_name, user_key, directory))
        # print("press 1 for WRITE new data\npress 2 for READ old data\npress 3 for REMOVE old data\npress 4 for QUIT")
        readdata_b = tkinter.Button(open_encrypter_main_menu_gui, bg="#20bebe", fg="white", width=30, padx=2, pady=2, font=("Arial", 18), text="Read Data", command=lambda: readdata.Reader(user_key, directory))
        deletedata_b = tkinter.Button(open_encrypter_main_menu_gui, bg="#20bebe", fg="white", width=30, padx=2, pady=2, font=("Arial", 18), text="Delete Data", command=lambda: deletedata.text_deleter(new_user, user_key, directory))
        Logout_b = tkinter.Button(open_encrypter_main_menu_gui, bg="#20bebe", fg="white", width=30, padx=2, pady=2, font=("Arial", 18), text="Log out", command=lambda: kill_login())
